...
default_session_state_dict = {
    "short_description_gpt": "",
...
def create_presentation(
    ...
    available_themes = ["data/theme1.pptx", "data/theme3.pptx", "data/theme4.pptx"]

    # Выбираем случайную тему из списка
    selected_theme = random.choice(available_themes)

    # Создаем презентацию и применяем выбранную тему
    prs = Presentation(selected_theme)
    prs.slide_master.slide_width = prs.slide_width
    prs.slide_master.slide_height = prs.slide_height

    slide_layouts = prs.slide_master.slide_layouts

    welcome_slide = prs.slides.add_slide(slide_layouts[0])
    title = welcome_slide.shapes.title
    subtitle = welcome_slide.placeholders[1]
    ...
    for slide_title, slide_description in slides_info:
        ...
        text_box = body.text_frame.add_paragraph()
        text_box.text = slide_description

        # Устанавливаем белый цвет текста
        title.text_frame.paragraphs[0].font.color.rgb = RGBColor(255, 255, 255)
        text_box.font.color.rgb = RGBColor(255, 255, 255)

        # Настраиваем заголовок и текст слайда
        left = Inches(5)
        top = Inches(-1.5)
        width = Inches(14)
        height = Inches(5)
...
def parse_financial_indicators(link: str):
    ...
    profitability_indicators = soup.find_all(
        "div", {"class": "cCard__BlockRating cCard__Padding10"}
    )
    # Выручка
...
with tab1:
        ...
        if len(row) == 0:
            ...
            try:
                (
                    ...
                    sales,
                    capital,
                ) = parse_financial_indicators(sbis_link)
                if text[-1] != ".":
                    text += "."
    ...
    st.caption(
        "Введите краткое или подробное описание стартапа. Оно преобразуется в короткое, отражающее суть."
    )
    short_description = st.text_area(
        "Введите краткое описание стартапа", key="short_description"
    )
    ...
    st.caption(
        "Введите описание проблемы, которую решает стартап в свободной форме. Предпочтительно подробно."
    )
    ...
    st.caption(
        "Введите в свободной форме подробно описание и ценностное предложение стартапа."
    )
    # st.markdown('''Пример''')
    ...
    st.title("Решение")
    st.caption(
        "Опишите подробно, в свободной форме функционал вашего решения. То, как он помогает закрыть потребности клиента."
    )
    ...
    st.title("Рынок")
    st.markdown(
        """Здесь мы вместе рассчитаем TAM, SAM, SOM вашего стартапа.
    ...
    st.caption(
        "Пожалуйста, выберите по какому географическому признаку исследовать рынок: РФ или Международный."
    )
    ...
    if st.button("Провести исследование", type="primary", key=5):
        ...
        sam_instruction = pa.market_sam_instruction(
            st.session_state["value_and_description_gpt"]
        )
        st.session_state["sam_instruction"] = sam_instruction

        # som_instruction = pa.market_som_instruction(value_and_description)
        som_instruction = pa.market_som_instruction(
            st.session_state["value_and_description_gpt"]
        )
    ...
    st.title("Конкуренты")
    st.caption(
        "Здесь автоматически находятся конкуренты и определяются ваши конкурентные преимущства"
    )
    if st.button("Найти конкурентов и определить преимущества", type="primary", key=6):
        competitors_list_gpt = pa.competitors_list(
            st.session_state["short_description_gpt"]
        )
        st.session_state["competitors_list_gpt"] = competitors_list_gpt
        competitive_advantages_gpt = pa.competitive_advantages(
            st.session_state["value_and_description_gpt"],
    ...
    st.caption(
        """Здесь мы вместе последовательно ответим на 3 вопроса, пользуясь предоставленным примером"""
    )

    st.subheader("Опишите способ генерации дохода и основные затраты вашего бизнеса?")
    if st.button("Привести пример и создать текст", key="bis_example_1"):
        profit_and_costs_example = pa.bis_model_profit_generation_and_costs(
            st.session_state["value_and_description_gpt"]
        )
    ...
    business_model_1 = st.text_area(
        "Способ генерации дохода и основные затраты", key="business_model_1"
    )

    st.subheader("Какие каналы вы используете для привлечения клиентов?")
    if st.button("Привести пример и создать текст", key="bis_example_2"):
        acquisition_example = pa.bis_model_acquisition_channels(
            st.session_state["value_and_description_gpt"]
        )
    ...
    business_model_2 = st.text_area(
        "Каналы для привлечения клиентов", key="business_model_2"
    )

    st.subheader("Как вы видите идите масштабирование своей бизнес-модели?")
    if st.button("Привести пример и создать текст", key="bis_example_3"):
        profit_and_costs_example = pa.bis_model_profit_generation_and_costs(
            st.session_state["value_and_description_gpt"]
        )
        st.session_state["bis_model_scaling_example"] = profit_and_costs_example
    st.markdown(st.session_state["bis_model_scaling_example"])
    business_model_3 = st.text_area(
        "Масштабирование бизнес-модели", key="business_model_3"
    )
    ...
    st.caption(
        "Введите годовую вырочку и количество клиентов, чтобы расчитать ARPU, churn rate, LT, LTV"
    )
    year_earnings = st.number_input(
        "Годовая выручка (млн. руб.): ", key="year_earnings"
    )
    clients_amount = st.number_input("Количество клиентов: ", key="clients_amount")
    if st.button("Рассчитать метрики", type="primary", key=8):
        try:
            metrics = pa.fin_metrics(
                st.session_state["value_and_description_gpt"],
                year_earnings,
                clients_amount,
            )
    ...
    if st.button("Привести пример", type="primary", key=9):
        traction_and_partners_example = pa.traction_and_partners(
            st.session_state["value_and_description_gpt"]
        )
        st.session_state[
            "traction_and_partners_example"
    ...
    traction_and_partners = st.text_area(
        "Трекшн и партнеры", key="traction_and_partners"
    )
    ...
    st.caption(
        "Опишите команду вашего проекта, бекграунд, текущих инвесторов и эдвайзеров"
    )
    ...
    st.caption(
        "Пожалуйста, в свободной форме опишите на каком инвестиционном раунде вы находитесь и на какие цели привлекаете инвестиции"
    )
    ...
    st.title("Контакты")
    st.caption(
        "В свободной форме напишите контакты, которые вы бы хотели продемонстрировать в презентации."
    )
    ...
    if agent_button:
        st.session_state["presentation_text"] = pa.create_presentation_text_agent(
            st.session_state
        )
        create_presentation(
            company_name,
            st.session_state["short_description_gpt"],
            st.session_state["problem_description_gpt"],
            st.session_state["solution_gpt"],
            "TAM: "
            + st.session_state["tam_summary"]
            + "\n"
            + "SAM: "
            + sam
            + "\n"
            ...
            + som,
            st.session_state["competitors_list_gpt"],
            st.session_state["competitive_advantages_gpt"],
            st.session_state["profit_and_costs_example"],
            st.session_state["acquisition_example"],
            st.session_state["bis_model_scaling_example"],
            ...
            + str(clients_amount)
            + "\n"
            + str(st.session_state["metrics"]),
            st.session_state["team_gpt_example"],
            st.session_state["roadmap_gpt"],
            st.session_state["contacts_gpt"],
        )
        # st.download_button(
        #     label="Download presentation",
        #     data=presentation_file.seek(0),
